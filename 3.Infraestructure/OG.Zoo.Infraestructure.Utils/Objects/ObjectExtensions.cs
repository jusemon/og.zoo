    ...
    public static class ObjectExtensions
    {
        ...
        )
        {
            return source
                ...
                .ToDictionary(propInfo => propInfo.Name, propInfo => GetValue(source, propInfo));
        }
        ...
        /// <param name="bindingAttr">The binding attribute.</param>
        /// <returns></returns>
        public static IDictionary<string, T> AsDictionary<T>(
            this object source,
            BindingFlags bindingAttr =
                BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance
        )
        {
            return source
                .GetType()
                .GetProperties(bindingAttr)
                .ToDictionary(propInfo => propInfo.Name, propInfo => (T)GetValue(source, propInfo));
        }
        ...
        {
            var converter = propInfo
                .GetCustomAttribute<FirestorePropertyAttribute>()
                ?.ConverterType;
            if (converter == null)
            {
            ...
            var documentRefereceConverter =
                (IFirestoreConverter<string>)ctor.Invoke(new object[] { });
